...
import { GoalsControl } from "./goals-control";
import { ExpansionSuite } from "./expansion-suite";
...
const paths: Record<IconName, React.ReactNode> = {
  ...
  target: <><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1"/><path d="m15.5 8.5 5-5M17 3.5h3.5V7"/></>,
};
...
const nav = [
  ...
  ["Goals", "target"], ["AI Command", "spark"], ["Automation", "clock"], ["Teams", "users"], ["Resources", "users"], ["Access", "lock"],
  ["Calendar", "calendar"], ["Budget", "wallet"], ["Finance", "wallet"], ["Risks", "risk"], ["Reliability", "pulse"],
  ["Views", "grid"], ["Knowledge", "folder"], ["Reports", "report"], ["Search", "search"], ["Integrations", "settings"], ["Administration", "shield"],
] as const;
...
function ModulePage({ active, projects, onNew, onInspect, notify, theme, setTheme, compactMode, setCompactMode, reducedMotion, setReducedMotion, canCreate, operations, operationCommand, currentEmail, governance, governanceCommand, reliability, reliabilityCommand, searchQuery }: { active: string; projects: typeof seedProjects; onNew: () => void; onInspect: (project: typeof seedProjects[number]) => void; notify: (message:string) => void; theme: "light"|"dark"|"system"; setTheme: (theme:"light"|"dark"|"system")=>void; compactMode:boolean; setCompactMode:(value:boolean)=>void; reducedMotion:boolean; setReducedMotion:(value:boolean)=>void; canCreate:boolean; operations:OperationData; operationCommand:(payload:Record<string,unknown>)=>Promise<boolean>; currentEmail:string; governance:GovernanceData; governanceCommand:(payload:Record<string,unknown>)=>Promise<boolean>; reliability:ReliabilityData; reliabilityCommand:(payload:Record<string,unknown>)=>Promise<boolean>; searchQuery:string }) {
  ...
  const titles: Record<string, [string,string]> = {
    ...
    Goals:["Goals & OKRs","Align portfolio execution to measurable outcomes and accountable owners."],
    "AI Command":["Nexus intelligence","Explainable project, risk, budget, report, and resource decision support."],
    Resources:["Resource management","Plan capacity, timesheets, leave, workload, and organizational skills."],
    Finance:["Financial operations","Control expenses, invoices, vendors, forecasts, and portfolio return."],
    Views:["Productivity views","Move between timeline, portfolio, mind map, and dependency perspectives."],
    Knowledge:["Knowledge hub","Build a governed wiki, knowledge base, templates, and versioned operating pages."],
    Search:["Universal search","Find projects, tasks, files, knowledge, people, and decisions across Nexus."],
  ...
  return <div className="module-page">
    ...
    {active==="Goals" && <GoalsControl notify={notify}/>}

    {["AI Command","Resources","Finance","Views","Knowledge","Search"].includes(active) && <ExpansionSuite active={active} projects={projects} tasks={operations.tasks} initialQuery={searchQuery} notify={notify}/>}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/api/session")
      ...
      .then(data => {
        ...
        window.setTimeout(() => {
          if (cancelled) return;
          if (data.user) localStorage.removeItem("nexus-open-workspace");
          else localStorage.setItem("nexus-open-workspace", "true");
          setSession({
            loading:false,
            user:data.user ?? {displayName:"Kshitij",email:"guest@nexus.local"},
          });
        }, wait);
      });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setAccessError("");
      if (session.user!.email === "guest@nexus.local" || (window.location.hostname === "terminal.local" && new URLSearchParams(window.location.search).has("qa"))) {
  ...
  const addProject = async () => {
    ...
    if (session.user?.email === "guest@nexus.local") {
      const project = {
        id: Date.now(),
      ...
      setProjectData((current) => [project, ...current]);
      setNewProjectName("");
      setNewProjectDescription("");
      setNewProjectOpen(false);
      setActive("Projects");
      setSavingProject(false);
      notify("Project created in the open workspace");
      return;
    }
    try {
      const response = await fetch("/api/projects", {
  ...
  if (session.loading || !session.user) return <main className="nexus-loader"><div className="loader-orbit"><i/><span className="brand-mark">N</span></div><div><strong>PROJECT NEXUS</strong><p>Preparing your command center</p><span className="loader-line"><i/></span></div></main>;
    ...
    <main className={`app-shell theme-${resolvedTheme} ${focusMode ? "focus-mode" : ""} ${compactMode ? "compact-mode" : ""} ${reducedMotion ? "reduced-motion" : ""}`}>
      ...
      <section className="workspace">
        <header className="topbar">
          ...
          <label className="search"><Icon name="search" size={17}/><input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")setActive("Search")}} placeholder="Search everything… press Enter" /></label>
          <div className="top-actions">
            ...
            <div className="profile" title={session.user.email==="guest@nexus.local"?"Open workspace":"Verified workspace account"}><div className="avatar">{session.user.displayName.split(" ").map(x=>x[0]).join("").slice(0,2).toUpperCase()}</div><div><strong>{session.user.displayName}</strong><small>{session.user.email==="guest@nexus.local"?"Open workspace":access.membership.role.replace(/^\w/, letter=>letter.toUpperCase())}</small></div><Icon name="arrow" size={14}/></div>
          </div>
        </header>
